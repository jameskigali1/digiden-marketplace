...
const Menu = ({ className }) => <Icon className={className}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="12" x2="21" y2="12"></line><line x1="3" y1="6" x2="21" y2="6"></line><line x1="3" y1="18" x2="21" y2="18"></line></svg></Icon>;
...
const Header = ({ onWhatsAppClick, scrollToSection, onMenuToggle }) => (
    <header className="fixed top-4 left-1/2 -translate-x-1/2 z-[1000] w-[95vw] max-w-7xl transition-all duration-500"
        style={{
            background: 'var(--card-background)',
            ...
            <div className="flex items-center justify-between h-16">
                ...
                <nav className="hidden md:flex items-center space-x-2">
                 ...
                 <div className="flex items-center gap-2">
                    <button onClick={onWhatsAppClick} className="hidden sm:block font-semibold rounded-full px-4 py-2 text-sm text-white"
                     style={{
                    ...
                    <button onClick={onMenuToggle} className="md:hidden p-2 rounded-full text-text-primary">
...
const MobileNav = ({ navItems, onLinkClick }) => (
    <div className="fixed inset-0 z-[999] bg-black/50 backdrop-blur-sm" onClick={() => onLinkClick(null, true)}>
        <div 
            className="fixed top-24 right-4 w-[90vw] max-w-sm p-6 rounded-3xl"
            style={{
            ...
            <nav className="flex flex-col space-y-4">
                {navItems.map(item => (
                    <button 
                        ...
                        className="text-lg font-semibold text-text-primary p-3 rounded-lg text-left hover:bg-accent-primary/10 transition-colors duration-300"
                    >
                        {item}
                    </button>
                ))}
...
const HeroSection = ({ onWhatsAppClick, scrollToSection }) => {
    return (
      <section className="min-h-screen relative overflow-hidden flex items-center justify-center text-center p-4 pt-24 pb-12"
      ...
      >
        <div className="relative z-10 p-4 fade-in-up">
            ...
            <button onClick={() => scrollToSection('categories')} className="text-black font-bold uppercase tracking-wider text-lg px-8 sm:px-12 py-4 sm:py-5 rounded-full transition-all duration-400 transform-style-preserve-3d"
              style={{
                background: 'linear-gradient(135deg, #FFD700 0%, #FF8F00 50%, #E91E63 100%)',
      ...
      <div className="glass-card overflow-hidden flex flex-col h-full">
          <div className="relative">
             <button onClick={() => onProductSelect(product)} className="w-full text-left block">
                <div className="relative h-56 md:h-64 overflow-hidden border-b-2" style={{borderColor: 'var(--accent-secondary)'}}>
          ...
          <div className="p-4 sm:p-6 flex flex-col flex-grow">
              <button onClick={() => onProductSelect(product)} className="w-full text-left block">
                <h4 className="text-lg sm:text-xl font-bold text-text-primary mb-3 h-14 overflow-hidden" style={{fontFamily: "'Playfair Display', serif"}}>{name}</h4>
              </button>
              <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center text-accent-secondary">
                      <Star className="w-5 h-5" />
                      <span className="ml-2 font-semibold text-base sm:text-lg text-text-secondary">{rating}</span>
                  </div>
                  <div className="text-right">
                      <span className="text-xl sm:text-2xl font-bold" style={{color: 'var(--accent-primary)'}}>₹{salePrice}</span>
                      <span className="ml-2 line-through text-sm sm:text-base text-text-tertiary">₹{price}</span>
                  </div>
              </div>
          </div>
 ...
 <section id="featured" className="py-16 md:py-24" style={{background: 'linear-gradient(180deg, var(--bg-primary) 0%, var(--bg-secondary) 100%)'}}>
  <div className="container mx-auto px-4">
   <h2 className="section-heading-3d">Featured Products</h2>
     <p className="text-center text-base md:text-lg text-text-secondary max-w-3xl mx-auto mb-12 md:mb-16">
 ...
 return (
  <section id="categories" className="py-16 md:py-24" style={{background: 'radial-gradient(ellipse at center, var(--bg-primary) 0%, var(--bg-secondary) 100%)'}}>
   <div className="container mx-auto px-4">
     ...
     <p className="text-center text-base md:text-lg text-text-secondary max-w-3xl mx-auto mb-12">
     ...
     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8" style={{perspective: '2000px'}}>
       {filteredCategories.map((category, index) => (
          <button 
            ...
            className="glass-card h-72 md:h-80 p-0 overflow-hidden"
...
const CategoryPage = ({ category, onWhatsAppClick, onProductBuyClick, onProductSelect }) => (
 <div className="category-page-container fade-in-up bg-bg-primary min-h-screen pt-24 md:pt-32">
  <div className="container mx-auto px-4 py-8 md:py-12">
   ...
   <div className="text-center mb-12 md:mb-16">
     ...
     <h1 className="text-4xl md:text-5xl font-extrabold text-text-primary tracking-tight">{category.name}</h1>
   ...
   <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8" style={{perspective: '1000px'}}>
     ...
     <div className="text-center mt-16 md:mt-24 p-8 md:p-10 rounded-3xl glass-card">
     <p className="text-2xl md:text-3xl font-bold text-text-primary mb-4">...Or Get ALL Products for Just ₹99!</p>
     <p className="text-base md:text-lg text-text-secondary mb-8 max-w-2xl mx-auto">Why buy one? Get all these, plus 200+ more from every category, in the full bundle.</p>
     <Button onClick={onWhatsAppClick} size="lg" className="text-black font-bold uppercase tracking-wider text-lg px-8 md:px-10 py-4 rounded-full transition-all duration-300"
           style={{
...
const ProductDetailPage = ({ product, onBack, onBuyClick }) => {
    ...
    return (
        <div className="product-page-container fade-in-up bg-bg-primary min-h-screen pt-24 md:pt-32">
                    ...
                    <div className="lg:w-3/5">
                        ...
                        <div className="my-6">
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex items-start"><Tag className="inline w-4 h-4 mr-2 text-green-600 mt-1 flex-shrink-0" /><span><b>Bank Offer</b> 5% cashback on HDFC Credit Card up to ₹100</span></div>
                                <div className="flex items-start"><Tag className="inline w-4 h-4 mr-2 text-green-600 mt-1 flex-shrink-0" /><span><b>Bank Offer</b> 10% instant discount on SBI Debit Card up to ₹200</span></div>
                                <div className="flex items-start"><Tag className="inline w-4 h-4 mr-2 text-green-600 mt-1 flex-shrink-0" /><span><b>Special Price</b> Get extra ₹300 off (price inclusive of cashback/coupon)</span></div>
                            </div>
                        </div>
...
function App() {
  ...
  const handleMobileLinkClick = (sectionId, shouldClose) => {
      if(shouldClose) {
          setIsMenuOpen(false);
          return;
      }
            ...
            />
             <section id="pricing" className="py-16 md:py-24 bg-bg-secondary">
               <div className="container mx-auto px-4">
                 ...
                 <p className="text-center text-base md:text-lg text-text-secondary max-w-3xl mx-auto mb-12 md:mb-16">Individual asset packs can cost ₹1000s. Get our entire curated library for a single, tiny payment.</p>
                 <div className="max-w-md mx-auto">
                   <div className="glass-card p-6 md:p-8">
                       <div className="text-center mb-8">
                         ...
                         <div className="text-text-tertiary line-through text-lg md:text-xl">₹20,000+ Value</div>
                         <div className="text-success-color font-semibold text-base md:text-lg">Save Over 99%!</div>
                       </div>
             ...
             <section id="reviews" className="py-16 md:py-24 bg-bg-primary">
               <div className="container mx-auto px-4">
                 ...
                 <p className="text-center text-base md:text-lg text-text-secondary max-w-3xl mx-auto mb-12 md:mb-16">Join thousands of satisfied customers who have supercharged their projects with DigiDen.</p>
  ...
  return (
    <div className="min-h-screen font-sans bg-bg-primary">
      ...
      <Header onWhatsAppClick={handleBundleBuyClick} scrollToSection={scrollToSection} onMenuToggle={() => setIsMenuOpen(!isMenuOpen)} />

      {isMenuOpen && <MobileNav navItems={['Categories', 'Featured', 'Pricing', 'Reviews']} onLinkClick={handleMobileLinkClick} />}
      ...
      {showModal && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-md p-6 md:p-8 relative fade-in-up">
